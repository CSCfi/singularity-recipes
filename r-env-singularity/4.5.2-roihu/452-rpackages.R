...
# package commented out without explanation -> installation moved to set 10, fails earlier
...
install_version("gargle", dependencies = FALSE, lib = libpath)
install_version("googledrive", dependencies = FALSE, lib = libpath)
install_version("ids", dependencies = FALSE, lib = libpath)
install_version("rematch2", dependencies = FALSE, lib = libpath)
...
configure.args='--with-Rmpi-include=/usr/mpi/gcc/openmpi-4.1.9a1/include --with-Rmpi-libpath=/usr/mpi/gcc/openmpi-4.1.9a1/lib64 --with-Rmpi-type=OPENMPI', lib = libpath)

install_version("doMPI", dependencies = FALSE, lib = libpath)

install_version("pbdMPI", dependencies = FALSE, 
configure.args='--with-mpi-include=/usr/mpi/gcc/openmpi-4.1.9a1/include --with-mpi-libpath=/usr/mpi/gcc/openmpi-4.1.9a1/lib64 --with-mpi-type=OPENMPI', 
lib = libpath)
...
setwd("/appl/soft/manual/aida/x86_64/r-env/452/452-source")
...
install_github("ingmbioinfo/combiroc", dependencies = FALSE, lib = libpath)
...
# INLA
# installed further down as a dependency of FishStatsUtils
# use works only in project-specific installations
# use requires the second installation step because of a Linux library version mismatch between INLA/r-env
# https://www.r-inla.org/download-install
# install.packages("INLA", repos = c(getOption("repos"), INLA = "https://inla.r-inla-download.org/R/stable"), dep = TRUE, lib = libpath)
# library(INLA)
# inla.binary.install(os = c("Rocky Linux-9"), path = NULL, verbose = TRUE, md5.check = TRUE)
...
# removed because a very large data package, slows down setting libpath <- .libPaths()[1]
...
# rstanarm (note: version should march rstan)
...
install_version("MetaSKAT", dependencies = FALSE, lib = libpath)
...
install_version("zeallot", lib = libpath, dependencies = FALSE)
...
# note: doesn't work like this in 4.5.1 or 4.5.2, see below (fix from a Github issue)
install_github("brendanf/optimotu", dep = FALSE, lib = libpath)

# cd /appl/soft/manual/aida/x86_64/r-env/452/452-source
...
# -B /appl/soft/manual/aida/x86_64/r-env:/appl/soft/manual/aida/x86_64/r-env \
# /appl/soft/manual/aida/x86_64/r-env/452/452.sif \
# R --no-save
# libpath <- .libPaths()[2]
...
# mia & miaViz
BiocManager::install("mia", lib = libpath, update = FALSE)
BiocManager::install("miaViz", lib = libpath, update = FALSE)
